...
export const filterAffairs = (affairs: AffairType[], filter: FilterType): AffairType[] => { // need to fix any
   if (filter==="all"){
      ...
      let filtraition =affairs.filter(f=>f.priority===filter)
       return filtraition;
   }
}
export const deleteAffair = (affairs: AffairType[], _id: number): AffairType[] => { // need to fix any
    let deletingAffair=affairs.filter(f=>f._id!==_id)
    return deletingAffair; // need to fix
}
...
function HW2() {
    ...
    const deleteAffairCallback = (_id: number) => {
        setAffairs(prevAffairs => prevAffairs.filter(affair => affair._id !== _id));
    }
